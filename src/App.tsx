...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        ...
        // By default, let's create a premium fallback mock session so users don't get blocked
        setUser({
          email: 'pbmmurty79@gmail.com',
          uid: 'uid-auditor',
          role: 'admin'
        });
        setIsAdminUnlocked(true); // Pre-unlock for extreme convenience inside testing mode
      }
    });
    return () => unsub();
  }, []);
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (adminPassword === 'admin123' || user?.email === 'pbmmurty79@gmail.com') {
      setIsAdminUnlocked(true);
      setAdminAuthError('');
      setActiveView('admin');
  ...
  return (
    <div id="vidi-vault-root" className="min-h-screen bg-[#0F172A] font-sans text-slate-200 flex flex-col justify-between">
      ...
      <header className="h-16 bg-[#151d2e] border-b border-slate-700 px-8 flex flex-wrap gap-4 items-center justify-between shadow-md">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="hidden sm:block">
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold">Security File Repository</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 max-w-7xl w-full mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#1E293B] border border-slate-700 p-4 rounded-lg space-y-3 shadow-md">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-slate-450 px-1">Security Folders</h3>
            <div className="space-y-1">
              {['All', 'Classical', 'Mass', 'Educational', 'Comedy', 'Tragedy', 'Devotional', 'Melody Class'].map((cat) => {
          ...
          {/* Text/Search Filter */}
          <div className="bg-[#1E293B] border border-slate-700 p-4 rounded-lg space-y-2 shadow-sm">
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-450">Fast Match Filter</label>
            <input
              type="text"
          ...
          <div className="bg-[#1E293B] border border-slate-700 p-4 rounded-lg space-y-3 shadow-sm">
              ...
              <form onSubmit={handleAdminLogin} className="space-y-4">
                <div className="space-y-1 text-left">
                  ...
                  <input
                    ...
                    placeholder="Enter security passport passphrase (admin123)"
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3 items-center justify-between border-b border-slate-700 pb-3">
                <div className="flex gap-2">
                  {[
                    { id: 'vault', label: 'Decryption Vault', icon: Play },
